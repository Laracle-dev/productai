...
// WebGL Orb Component
const Orb = ({ isAnimating = true, hue = 240, width = "100%", height = "100%" }) => {
  const ctnDom = useRef(null);
  
  // Classes needed for the WebGL implementation
  class Vec3 {
  ...
  useEffect(() => {
    ...
    const update = (t) => {
      ...
      // Force hover effect when isAnimating is true for AI responses
      const effectiveHover = isAnimating ? 1 : targetHover;
      program.uniforms.hover.value +=
        (effectiveHover - program.uniforms.hover.value) * 0.1;

      // Continuously rotate when animated for AI responses
      if (isAnimating) {
...
const EnergySphere = ({ isAnimating = true }) => {
  return (
    <Orb isAnimating={isAnimating} hue={240} width="120px" height="120px" />
  );
};
